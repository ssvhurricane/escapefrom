...
{
    public class WolfHealingAbility : IAbilityWithOutParam
    {
        ...
        public WolfHealingAbility(SignalBus signalBus,
        ...
        public void StartAbility(IPresenter ownerPresenter, ActionModifier actionModifier)
        {
            ...
            if (presenter != null)
            {
                var view = (WolfView)presenter.GetView();

                if (actionModifier == ActionModifier.None)
                {
                    // ToDo...
                    Debug.Log("[WolfHealingAbility]-> Base Abilty!");
